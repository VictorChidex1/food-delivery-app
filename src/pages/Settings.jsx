...
const Settings = () => {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setFormData(prev => ({
        ...
        email: user.email || "",
        // If you saved phone in login/signup, you'd load it here too
        // Otherwise it defaults to empty or you can keep a placeholder
      }));
    }
  }, []);
  ...
  const handleSave = (section) => {
    // OPTIONAL: Update localStorage with new data so it persists across refreshes
    if (section === 'Personal') {
      const updatedUser = { fullName: formData.fullName, email: formData.email };
      localStorage.setItem('user', JSON.stringify(updatedUser));
      // You might want to trigger a re-render of the Navbar here using a Context or window reload
      // window.location.reload(); // Simple way to update Navbar name
    }
    ...
    <div className="max-w-4xl mx-auto p-6 pb-20">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-red-50 rounded-2xl border border-red-100 p-6">
          ...
          <button className="w-full bg-white border border-red-200 text-red-600 px-4 py-2.5 rounded-lg font-medium hover:bg-red-50 transition-colors flex items-center justify-center gap-2">
            <Trash2 size={18} />
            Delete Account
          </button>
        </div>
