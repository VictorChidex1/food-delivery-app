...
import { Search, ShoppingBag, MapPin, Menu, X, ChevronDown, Box, Heart, CreditCard, Settings, HelpCircle, LogOut } from 'lucide-react';
    ...
    >
      <div className="max-w-7xl mx-auto flex h-20 items-center justify-between px-4 md:px-6 gap-4">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          {user ? (
            <div className="relative" ref={profileDropdownRef}>
              ...
              >
                ...
                <div className="text-left hidden lg:block">
                  <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wide">Hello,</p>
            ...
            <div className="flex items-center gap-2">
               <Link to="/login">
                 <Button variant="ghost" className="font-bold text-gray-700 hover:text-[#FF5200]">Log in</Button>
               </Link>
               <Link to="/signup">
                 <Button className="rounded-full font-bold px-6 shadow-lg shadow-orange-200 transition-transform hover:scale-105">Sign Up</Button>
               </Link>
            </div>
          )}
        ...
        <div className="flex md:hidden items-center gap-3">
          ...
          <Button variant="ghost" size="icon" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-gray-900">
            ...
            <div className="p-4 flex flex-col gap-4">
              ...
              {user ? (
                <div className="flex flex-col gap-1">
                  ...
                  <button onClick={handleLogout} className="flex items-center gap-3 px-3 py-3 text-red-500 hover:bg-red-50 rounded-lg mt-2 transition-colors">
                    <LogOut size={18} />
                    <span className="text-sm font-bold">Log Out</span>
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-3 mt-2">
                  <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="w-full py-3 border border-gray-200 rounded-xl font-bold text-gray-700 text-center hover:bg-gray-50">
                    Log In
                  </Link>
                  <Link to="/signup" onClick={() => setIsMobileMenuOpen(false)} className="w-full py-3 bg-[#FF5200] text-white rounded-xl font-bold shadow-md text-center hover:bg-orange-600">
                    Sign Up
                  </Link>
                </div>
              )}
