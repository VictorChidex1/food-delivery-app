...
const Navbar = () => {
  ...
  const getUserName = () => {
    ...
    if (user?.fullName) return user.fullName; // Fallback for old local data structure if any
        ...
        <div className="hidden md:flex items-center gap-4">
          {user ? (
            ...
            <div className="flex items-center gap-3">
              <Link
                to="/login"
                className="text-gray-700 font-bold hover:text-[#FF5200] transition-colors"
              >
                Log in
              </Link>
              <Link
                to="/signup"
                className="bg-[#FF5200] text-white px-5 py-2.5 rounded-full text-sm font-bold hover:bg-orange-600 shadow-lg shadow-orange-200 transition-transform hover:-translate-y-0.5"
              >
                Sign Up
              </Link>
            </div>
        ...
        <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-gray-100 shadow-xl p-4 flex flex-col gap-4 animate-in slide-in-from-top-5">
          ...
          {user ? (
            <div className="flex flex-col gap-1">
              ...
              <button
                onClick={handleLogout}
                className="flex items-center gap-3 px-3 py-3 text-red-500 hover:bg-red-50 rounded-lg mt-2"
              >
                <LogOut size={18} />
                <span className="text-sm font-bold">Log Out</span>
              </button>
            ...
            <div className="flex flex-col gap-3 mt-2">
              <Link
                to="/login"
                className="w-full py-3 border border-gray-200 rounded-xl font-bold text-gray-700 text-center"
              >
                Log In
              </Link>
              <Link
                to="/signup"
                className="w-full py-3 bg-[#FF5200] text-white rounded-xl font-bold shadow-md text-center"
              >
                Sign Up
              </Link>
            </div>
